...
修炼相关行动不得在行动模板中写死 raw 修为收益。行动模板应通过 `ActionCultivationProfile` 提供相对当前 `RealmSegment` 的效率、兼容小时策略、F1 资格、事件中断策略、压力增长、风险暴露和资源槽数量。

数值方向：

```text
闭关修炼：兼容小时 full，可接资源输入，可参与 F1；
普通吐纳：兼容小时 full，风险低，效率低于标准主动路径；
显式托管：安全低收益，满足条件时可参与 F1；
自动兜底：极低收益，force_speed=N1，f1_eligible=false。
```
...
| 行动通过 `ActionCultivationProfile` 提供相对效率 | 行动模板直接写死 raw 修为收益 | 修炼收益由当前境界段基准反推，行动只提供相对效率与结算策略 |
