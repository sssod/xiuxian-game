...
5. Resource input fields are present on commands, but non-empty resource inputs are rejected until inventory/resource ownership exists; this preserves the design rule that cultivation resources must enter through explicit command resource input and cannot become instant-use shortcuts.
...
| Runtime architecture | In progress | 80% | Runtime skeletons for room, world time, command queue, character state, settlement, result package, replay, data, save, logging, and UI adapter added; save/load recovery runner, save-slot diagnostics, first-entry character initialization, command validation, fallback, hourly cultivation settlement, MethodState persistence, and command-template settlement branching added | Add scenario-focused settlement APIs as P3/P4 systems arrive | None |
...
| Character/realm/cultivation | In progress | 55% | Config-backed true-spirit/current-life initialization added with identity, attributes, aptitude, derived bars, lifespan, initial cultivation state, initial MethodState, replay event, save/load preservation, debug UI summary, active breathing, seclusion cultivation, method study, consolidation, managed action, rest/recovery, hourly CP or mastery settlement, Qi Refining segment progress, minor-stage advancement, and low-value fallback meditation | Add resource input ownership/consumption, residual effects, recovery limits, and richer method/main-method bindings as inventory/resource state arrives | Final balance values still need later config tables |
...
| UI integration | In progress | 35% | Low-fidelity debug boot scene now exposes build/config, room/time, character summary, method summary, queue capacity, current command/fallback/managed state, active breathing, seclusion, method study, consolidation, managed action and rest enqueue controls, save-slot diagnostics, save/load results, result summary, and replay/result counts | Build cultivation and command management views as remaining P3 state APIs arrive | Needs resource input UI and richer command editing APIs |
| Validation/replay | In progress | 60% | Empty-room smoke plus save/load/F1 repeated-tick smoke now exercise result packages, replay entries, character initialization preservation, interrupted-settlement recovery, load failure diagnostic codes, active-breathing CP gain, minor-stage advancement, command-template settlement, resource-input rejection, MethodState mastery gain, consolidation pressure/foundation changes, managed-action state, rest recovery, and fallback after command completion | Add fixed-seed gameplay scenarios as P3/P5 systems arrive | Needs resource input ownership and residual-effect semantics |
