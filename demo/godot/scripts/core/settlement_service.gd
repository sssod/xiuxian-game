...
static func settle_turn(runtime: Dictionary, content: Dictionary, player_id := DemoConstants.LOCAL_PLAYER_ID) -> Dictionary:
	...
	var ledger_state_deltas := {}
	var true_spirit_deltas := {}
	var demo_summary_delta := {}
	var asset_log_entries := []
	var character_log_entries := []
	var sect_log_entries := []
	var sect_ai_outcomes := []
	var breakthrough_outcomes := []
	...
	for action in actual_queue:
		...
		elif action_id == "prepare_breakthrough":
			var prepare_result := _settle_prepare_breakthrough_action(runtime, content, action, turn_id, world_day, cursor_world_hour, remaining_hours, character_deltas, node_deltas, asset_container_deltas, item_instance_deltas, asset_log_entries, character_log_entries)
			remaining_hours = int(prepare_result["remaining_hours"])
			cursor_world_hour = int(prepare_result["cursor_world_hour"])
			actual_entries.append(prepare_result["actual_entry"])
			visible_logs.append_array(prepare_result["visible_logs"])
			hidden_world_logs.append_array(prepare_result["hidden_world_logs"])
			rumor_entries.append_array(prepare_result["rumor_entries"])
		elif action_id == "attempt_breakthrough":
			var breakthrough_result := _settle_attempt_breakthrough_action(runtime, content, action, turn_id, world_day, cursor_world_hour, remaining_hours, character_deltas, node_deltas, sect_deltas, asset_container_deltas, item_stack_deltas, item_instance_deltas, active_resource_effect_deltas, ledger_state_deltas, true_spirit_deltas, demo_summary_delta, asset_log_entries, character_log_entries, sect_log_entries)
			remaining_hours = int(breakthrough_result["remaining_hours"])
			cursor_world_hour = int(breakthrough_result["cursor_world_hour"])
			actual_entries.append(breakthrough_result["actual_entry"])
			visible_logs.append_array(breakthrough_result["visible_logs"])
			hidden_world_logs.append_array(breakthrough_result["hidden_world_logs"])
			rumor_entries.append_array(breakthrough_result["rumor_entries"])
			breakthrough_outcomes.append_array(breakthrough_result["breakthrough_outcomes"])
		else:
	...
	var summary := {
		...
		"breakthrough_outcomes": breakthrough_outcomes,
		"notes": _summary_notes(interruptions, rumor_entries, sect_ai_outcomes, breakthrough_outcomes)
	}
	...
	debug_logs.append(LogUtils.debug("s1_package_created", "ResultPackage generated by Phase E settlement.", turn_id, next_world_hour, {
		...
		"breakthrough_outcome_count": breakthrough_outcomes.size()
	}))

	var package := {
		"package_id": "s1_turn_%03d_phase_e" % turn_id,
		...
		"state_deltas": {
			...
			"item_stacks": item_stack_deltas,
				...
				"ledger_states": ledger_state_deltas,
				"true_spirits": true_spirit_deltas,
				"demo_summary": demo_summary_delta,
...
static func _settle_prepare_breakthrough_action(runtime: Dictionary, content: Dictionary, action: Dictionary, turn_id: int, world_day: int, cursor_world_hour: int, remaining_hours: int, character_deltas: Dictionary, node_deltas: Dictionary, asset_container_deltas: Dictionary, item_instance_deltas: Dictionary, asset_log_entries: Array, character_log_entries: Array) -> Dictionary:
	var planned_hours := int(action.get("planned_duration_hours", 0))
	var consumed_hours = min(planned_hours, remaining_hours)
	var ended_hour: int = cursor_world_hour + int(consumed_hours)
	var status := "completed"
	...
	var actor_id := str(action.get("actor_id", ""))
	var event := ContentLoader.event_template(content, "breakthrough_preparation_shortage")
	var container_id := _inventory_container_id(runtime, character_deltas, actor_id)
	var granted_instance_ids := []
	for item_template_id in event.get("reward_item_instances", []):
		var item_id := str(_find_available_item_instance_id(runtime, item_instance_deltas, actor_id, str(item_template_id)))
		if item_id == "":
			var template := ContentLoader.item_template(content, str(item_template_id))
			if template.is_empty():
				continue
			item_id = "instance_%s_t%03d_h%06d" % [str(item_template_id), turn_id, ended_hour]
			item_instance_deltas[item_id] = {
				"item_id": item_id,
				"item_template_id": str(item_template_id),
				"tier_code": template.get("tier_code", "M0"),
				"quality_code": template.get("quality_code", "rare"),
				"instance_state": {
					"key_item_kind": "breakthrough_prerequisite",
					"source_event_id": "breakthrough_preparation_shortage",
					"source_turn_id": turn_id,
					"source_world_hour": ended_hour,
					"consumed": false
				},
				"durability": 1.0,
				"charge": 0.0,
				"source_log_refs": [],
				"owner_container_ref": {"container_type": "CharacterInventory", "container_id": container_id},
				"tags": ["BreakthroughKeyItem"]
			}
			_append_asset_container_delta(asset_container_deltas, container_id, "item_instance_refs", item_id)
		granted_instance_ids.append(item_id)

	for revealed_node in event.get("reveal_nodes", []):
		_set_node_delta(node_deltas, str(revealed_node), {"visibility_state": "known"})

	var character := _character_with_deltas(runtime, character_deltas, actor_id)
	var identity: Dictionary = character.get("sect_identity", {}).duplicate(true)
	var authorizations: Array = identity.get("special_authorizations", []).duplicate(true)
	var support_authorization := str(event.get("support_authorization", "demo_breakthrough_support"))
	if support_authorization != "" and not authorizations.has(support_authorization):
		authorizations.append(support_authorization)
	identity["special_authorizations"] = authorizations
	_set_character_delta(character_deltas, actor_id, {
		"sect_identity": identity,
		"breakthrough_preparation_state": {
			"prepared_turn_id": turn_id,
			"prepared_world_hour": ended_hour,
			"support_authorization": support_authorization,
			"key_item_instance_refs": granted_instance_ids,
			"remaining_gap_refs": _breakthrough_gap_refs(runtime, character_deltas, item_instance_deltas, actor_id)
		}
	})

	var asset_log := _asset_log("breakthrough_key_granted", actor_id, container_id, "breakthrough_preparation_shortage", action.get("instruction_id", ""), turn_id, world_day, ended_hour, consumed_hours, {
		"item_instances": granted_instance_ids,
		"support_authorization": support_authorization
	})
	asset_log_entries.append(asset_log)
	character_log_entries.append(LogUtils.visible("breakthrough_prepared", "完成破境准备清点，关键物与宗门护持记录已建立。", turn_id, world_day, ended_hour))
	visible_logs.append(LogUtils.visible("breakthrough_prepared", "破境准备完成：试炼令、破境引与宗门护持记录已写入背包 / 角色状态。", turn_id, world_day, ended_hour))
	hidden_world_logs.append(LogUtils.visible("world_breakthrough_preparation", "%s 完成压缩破境准备。" % actor_id, turn_id, world_day, ended_hour))
	rumor_entries.append({
		"rumor_id": "breakthrough_preparation_shortage_t%03d_h%06d" % [turn_id, ended_hour],
		"event_id": "breakthrough_preparation_shortage",
		"source_node_id": _target_node(action),
		"turn_id": turn_id,
		"world_hour": ended_hour,
		"message": event.get("rumor_text", "破境准备有了眉目。")
	})
	actual_entry["granted_item_instances"] = granted_instance_ids
	actual_entry["support_authorization"] = support_authorization
	return {
		"remaining_hours": remaining_hours - consumed_hours,
		"cursor_world_hour": ended_hour,
		"actual_entry": actual_entry,
...
static func _settle_attempt_breakthrough_action(runtime: Dictionary, content: Dictionary, action: Dictionary, turn_id: int, world_day: int, cursor_world_hour: int, remaining_hours: int, character_deltas: Dictionary, node_deltas: Dictionary, sect_deltas: Dictionary, asset_container_deltas: Dictionary, item_stack_deltas: Dictionary, item_instance_deltas: Dictionary, active_resource_effect_deltas: Dictionary, ledger_state_deltas: Dictionary, true_spirit_deltas: Dictionary, demo_summary_delta: Dictionary, asset_log_entries: Array, character_log_entries: Array, sect_log_entries: Array) -> Dictionary:
	var template := ContentLoader.action_template(content, "attempt_breakthrough")
	var planned_hours := int(action.get("planned_duration_hours", template.get("fixed_duration_hours", 48)))
	var visible_logs := []
	var hidden_world_logs := []
	var rumor_entries := []
	var breakthrough_outcomes := []
	if remaining_hours < planned_hours:
		var skipped := _skipped_entry(action, "fixed_duration_breakthrough_requires_full_remaining_budget")
		return {
			"remaining_hours": remaining_hours,
			"cursor_world_hour": cursor_world_hour,
			"actual_entry": skipped,
			"visible_logs": [LogUtils.visible("breakthrough_skipped", "突破行动需要完整固定时段，当前剩余时间不足，未创建事件链。", turn_id, world_day, cursor_world_hour)],
			"hidden_world_logs": hidden_world_logs,
			"rumor_entries": rumor_entries,
			"breakthrough_outcomes": breakthrough_outcomes
		}

	var consumed_hours := planned_hours
	var ended_hour: int = cursor_world_hour + consumed_hours
	var actual_entry := _actual_entry(action, planned_hours, consumed_hours, "completed", "", cursor_world_hour, ended_hour)
	var actor_id := str(action.get("actor_id", ""))
	var required_items: Array = template.get("required_item_instance_template_ids", [])
	var consumed_item_ids := []
	for item_template_id in required_items:
		var item_id := _find_available_item_instance_id(runtime, item_instance_deltas, actor_id, str(item_template_id))
		if item_id == "":
			actual_entry["status"] = "skipped"
			actual_entry["skip_or_interrupt_reason"] = "breakthrough_key_item_missing:%s" % str(item_template_id)
			return {
				"remaining_hours": remaining_hours - consumed_hours,
				"cursor_world_hour": ended_hour,
				"actual_entry": actual_entry,
				"visible_logs": [LogUtils.visible("breakthrough_failed_to_start", "关键突破物不足，突破事件链未能成立。", turn_id, world_day, ended_hour)],
				"hidden_world_logs": hidden_world_logs,
				"rumor_entries": rumor_entries,
				"breakthrough_outcomes": breakthrough_outcomes
			}
		consumed_item_ids.append(item_id)

	for item_id in consumed_item_ids:
		_mark_item_instance_consumed(runtime, item_instance_deltas, str(item_id), turn_id, world_day, cursor_world_hour, action.get("instruction_id", ""), actor_id, asset_log_entries)

	var resource_bonus: Dictionary = _consume_action_resource_inputs(runtime, content, action, actor_id, turn_id, world_day, cursor_world_hour, ended_hour, consumed_hours, item_stack_deltas, active_resource_effect_deltas, asset_log_entries, character_deltas)
	var outcome := _resolve_breakthrough_outcome(runtime, content, action, actor_id, consumed_item_ids, resource_bonus, character_deltas)
	_apply_breakthrough_outcome(runtime, content, action, actor_id, outcome, turn_id, world_day, ended_hour, character_deltas, ledger_state_deltas, true_spirit_deltas, demo_summary_delta, character_log_entries, sect_log_entries)
	_set_node_delta(node_deltas, _target_node(action), {"append_state_tags": ["breakthrough_chain_recent"]})

	var log_kind := "breakthrough_succeeded"
	var visible_message := "破境成功，demo 压缩突破目标完成。"
	if str(outcome.get("result", "")) == "failure_survived":
		log_kind = "breakthrough_failed_survived"
		visible_message = "破境失败但保住性命，原因：%s。" % str(outcome.get("fail_reason_tags", []))
	elif str(outcome.get("result", "")) == "severe_injury_interrupted":
		log_kind = "breakthrough_interrupted_severe"
		visible_message = "破境被重伤中断，原因：%s。" % str(outcome.get("fail_reason_tags", []))
	visible_logs.append(LogUtils.visible(log_kind, visible_message, turn_id, world_day, ended_hour))
	hidden_world_logs.append(LogUtils.visible("world_breakthrough_chain_resolved", "%s 突破链结算：%s。" % [actor_id, outcome.get("result", "")], turn_id, world_day, ended_hour))
	var event := ContentLoader.event_template(content, "breakthrough_chain_basic")
	rumor_entries.append({
		"rumor_id": "breakthrough_chain_basic_t%03d_h%06d" % [turn_id, ended_hour],
		"event_id": "breakthrough_chain_basic",
		"source_node_id": _target_node(action),
		"turn_id": turn_id,
		"world_hour": ended_hour,
		"message": event.get("rumor_text", "一名外门弟子的突破结果被宗门记录。")
	})
	actual_entry["breakthrough_event_chain_id"] = "breakthrough_chain_basic"
	actual_entry["breakthrough_result"] = outcome.get("result", "")
	actual_entry["fail_reason_tags"] = outcome.get("fail_reason_tags", [])
	actual_entry["consumed_key_item_instance_refs"] = consumed_item_ids
	actual_entry["resource_bonus"] = resource_bonus
	actual_entry["breakthrough_choices"] = action.get("breakthrough_choices", [])
	actual_entry["stage_after"] = outcome.get("stage_after", "")
	breakthrough_outcomes.append(outcome)
	return {
		"remaining_hours": remaining_hours - consumed_hours,
		"cursor_world_hour": ended_hour,
		"actual_entry": actual_entry,
...
static func _breakthrough_gap_refs(runtime: Dictionary, character_deltas: Dictionary, item_instance_deltas: Dictionary, actor_id: String) -> Array:
	var gaps := []
	var character := _character_with_deltas(runtime, character_deltas, actor_id)
	...
	var method_state: Dictionary = runtime.get("method_states", {}).get(str(cultivation.get("main_dao_method_ref", "")), {})
	if str(method_state.get("current_effective_cap_stage", "")) != "A-3":
		gaps.append("main_method_cap_below_A-3")
	if _find_available_item_instance_id(runtime, item_instance_deltas, actor_id, "trial_token_instance") == "":
		gaps.append("trial_token_instance")
	if _find_available_item_instance_id(runtime, item_instance_deltas, actor_id, "breakthrough_catalyst_instance") == "":
		gaps.append("breakthrough_catalyst_instance")
	return gaps

static func _find_available_item_instance_id(runtime: Dictionary, item_instance_deltas: Dictionary, actor_id: String, item_template_id: String) -> String:
	...
	var refs: Array = runtime.get("asset_containers", {}).get(container_id, {}).get("item_instance_refs", []).duplicate(true)
	for item_id in item_instance_deltas.keys():
		var instance: Dictionary = item_instance_deltas[item_id]
		if str(instance.get("owner_container_ref", {}).get("container_id", "")) == container_id and not refs.has(str(item_id)):
			refs.append(str(item_id))
	for item_id in refs:
		var instance: Dictionary = runtime.get("item_instances", {}).get(str(item_id), {}).duplicate(true)
		if item_instance_deltas.has(str(item_id)):
			instance = item_instance_deltas[str(item_id)].duplicate(true)
...
static func _mark_item_instance_consumed(runtime: Dictionary, item_instance_deltas: Dictionary, item_id: String, turn_id: int, world_day: int, world_hour: int, instruction_id: String, actor_id: String, asset_log_entries: Array) -> void:
	var instance: Dictionary = runtime.get("item_instances", {}).get(item_id, {}).duplicate(true)
	if item_instance_deltas.has(item_id):
		instance = item_instance_deltas[item_id].duplicate(true)
	if instance.is_empty():
		return
	var state: Dictionary = instance.get("instance_state", {}).duplicate(true)
	state["consumed"] = true
	state["consumed_turn_id"] = turn_id
	state["consumed_world_hour"] = world_hour
	state["consumed_by_action_ref"] = instruction_id
	instance["instance_state"] = state
	item_instance_deltas[item_id] = instance
	var container_id := str(instance.get("owner_container_ref", {}).get("container_id", ""))
	asset_log_entries.append(_asset_log("breakthrough_key_consumed", actor_id, container_id, str(instance.get("item_template_id", item_id)), instruction_id, turn_id, world_day, world_hour, 0, {
		"item_instance_id": item_id,
		"consume_timing": "event_chain_created"
	}))

static func _resolve_breakthrough_outcome(runtime: Dictionary, content: Dictionary, action: Dictionary, actor_id: String, consumed_item_ids: Array, resource_bonus: Dictionary, character_deltas: Dictionary) -> Dictionary:
	var event := ContentLoader.event_template(content, "breakthrough_chain_basic")
	var character := _character_with_deltas(runtime, character_deltas, actor_id)
	var cultivation: Dictionary = character.get("cultivation", {})
	var method_state: Dictionary = runtime.get("method_states", {}).get(str(cultivation.get("main_dao_method_ref", "")), {})
	...
	if _stage_allows(str(method_state.get("current_effective_cap_stage", "")), "A-3"):
		score += 1
	else:
		_append_unique(fail_tags, "foundation_unstable")
	if consumed_item_ids.size() >= 2:
		score += 2
	else:
		_append_unique(fail_tags, "resource_shortage")
	var node_id := _target_node(action)
	if node_id in ["node_outer_sect_room", "node_abandoned_cave"]:
		score += 1
	else:
		_append_unique(fail_tags, "location_mismatch")
	if _has_special_authorization(character, "demo_breakthrough_support"):
		score += 1
	else:
		_append_unique(fail_tags, "outside_interference")
	if _resource_bonus_has(resource_bonus, "stability_talisman"):
		score += 1
	else:
		_append_unique(fail_tags, "mind_shaken")
	if float(bars.get("bar_mind_current", 0.0)) >= 60.0:
		score += 1
	else:
		_append_unique(fail_tags, "mind_shaken")

	var choices: Array = action.get("breakthrough_choices", [])
	if choices.has("stabilize_foundation"):
		score += 1
	if choices.has("follow_yunlu_formula") and _has_special_authorization(character, "demo_breakthrough_support"):
		score += 1
	if choices.has("hold_inner_breath") and float(bars.get("bar_qi_current", 0.0)) >= 30.0:
		score += 1
	if choices.has("force_meridian"):
		score -= 3
		_append_unique(fail_tags, "foundation_unstable")
		_append_unique(fail_tags, "mind_shaken")
	if choices.has("abandon_guard"):
		score -= 2
		_append_unique(fail_tags, "outside_interference")
	...
	if score >= success_threshold:
		...
		fail_tags = []
	elif score >= survive_threshold:
		result = "failure_survived"
		if fail_tags.is_empty():
			fail_tags.append("foundation_unstable")
	elif fail_tags.is_empty():
		fail_tags.append("foundation_unstable")

	return {
		"event_chain_id": "breakthrough_chain_basic",
		"actor_id": actor_id,
		"result": result,
		"score": score,
		"fail_reason_tags": fail_tags,
		"stage_before": str(cultivation.get("current_stage_code", "")),
		"stage_after": stage_after,
		"choice_ids": choices,
		"consumed_key_item_instance_refs": consumed_item_ids,
		"resource_bonus": resource_bonus,
		"target_node_id": node_id
	}

static func _apply_breakthrough_outcome(runtime: Dictionary, content: Dictionary, action: Dictionary, actor_id: String, outcome: Dictionary, turn_id: int, world_day: int, ended_hour: int, character_deltas: Dictionary, ledger_state_deltas: Dictionary, true_spirit_deltas: Dictionary, demo_summary_delta: Dictionary, character_log_entries: Array, sect_log_entries: Array) -> void:
	var character := _character_with_deltas(runtime, character_deltas, actor_id)
	var cultivation: Dictionary = character.get("cultivation", {}).duplicate(true)
	var bars: Dictionary = character.get("derived_bars", {}).duplicate(true)
	var injuries: Array = character.get("injuries_and_conditions", []).duplicate(true)
	var result := str(outcome.get("result", ""))
	if result == "success":
		cultivation["current_stage_code"] = "B-1"
		cultivation["cultivation_progress"] = 0.0
		cultivation["bottleneck_state"] = "none"
		cultivation["progress_cap_context"] = {
			"cap_source": "demo_breakthrough_chain",
			"theoretical_max_cap_stage": "B-1",
			"current_effective_cap_stage": "B-1",
		...
		if result == "severe_injury_interrupted":
			...
			injuries.append({
				...
				"severity": "severe",
				"fail_reason_tags": outcome.get("fail_reason_tags", []),
				"created_turn_id": turn_id,
				"created_world_hour": ended_hour
			})
			bars["bar_vitality_current"] = max(20.0, float(bars.get("bar_vitality_current", 100.0)) - 45.0)
			bars["bar_mind_current"] = max(10.0, float(bars.get("bar_mind_current", 80.0)) - 35.0)

	var history: Array = character.get("breakthrough_history", []).duplicate(true)
	var history_entry := outcome.duplicate(true)
	history_entry["turn_id"] = turn_id
	history_entry["world_day"] = world_day
	history_entry["world_hour"] = ended_hour
	history.append(history_entry)
	_set_character_delta(character_deltas, actor_id, {
		"cultivation": cultivation,
		"derived_bars": bars,
		"injuries_and_conditions": injuries,
		"breakthrough_history": history
	})

	var ledger_id := "ledger_local_stub"
	var ledger: Dictionary = runtime.get("ledger_states", {}).get(ledger_id, {}).duplicate(true)
	if ledger_state_deltas.has(ledger_id):
		ledger = ledger_state_deltas[ledger_id].duplicate(true)
	if ledger.is_empty():
		ledger = {"ledger_id": ledger_id, "spirit_id": DemoConstants.LOCAL_SPIRIT_ID, "entries": [], "status": "placeholder"}
	var entries: Array = ledger.get("entries", []).duplicate(true)
	entries.append({
		"entry_id": "ledger_breakthrough_t%03d_h%06d" % [turn_id, ended_hour],
		"entry_kind": "demo_breakthrough_result",
		"turn_id": turn_id,
		"world_day": world_day,
		"world_hour": ended_hour,
		"actor_id": actor_id,
		"result": result,
		"stage_after": outcome.get("stage_after", ""),
		"fail_reason_tags": outcome.get("fail_reason_tags", []),
		"consumed_key_item_instance_refs": outcome.get("consumed_key_item_instance_refs", [])
	})
	ledger["entries"] = entries
	ledger["status"] = "active_stub"
	ledger_state_deltas[ledger_id] = ledger

	if result == "success":
		var spirit: Dictionary = runtime.get("true_spirits", {}).get(DemoConstants.LOCAL_SPIRIT_ID, {}).duplicate(true)
		spirit["highest_stage_reached"] = "B-1"
		true_spirit_deltas[DemoConstants.LOCAL_SPIRIT_ID] = spirit

	demo_summary_delta["status"] = "completed"
	demo_summary_delta["completed_turn_id"] = turn_id
	demo_summary_delta["completed_world_hour"] = ended_hour
	...
	var character_message := "压缩突破结算：%s，原因标签：%s。" % [result, str(outcome.get("fail_reason_tags", []))]
	character_log_entries.append(LogUtils.visible("breakthrough_result", character_message, turn_id, world_day, ended_hour))
	var sect_message := "弟子%s压缩突破结算：%s。" % [actor_id, result]
	sect_log_entries.append(LogUtils.visible("sect_breakthrough_summary", sect_message, turn_id, world_day, ended_hour))

static func _resource_bonus_has(resource_bonus: Dictionary, resource_ref: String) -> bool:
	for entry in resource_bonus.get("consumed_resources", []):
		if typeof(entry) == TYPE_DICTIONARY and str(entry.get("resource_ref", "")) == resource_ref:
			return true
	return false

static func _has_special_authorization(character: Dictionary, authorization: String) -> bool:
	return character.get("sect_identity", {}).get("special_authorizations", []).has(authorization)
...
static func _summary_notes(interruptions: Array, rumor_entries: Array, sect_ai_outcomes: Array, breakthrough_outcomes: Array) -> Array:
	var notes := ["Phase E settlement path: personal actions, resources, node slots, sect AI, and breakthrough chain resolved through ResultPackage writeback."]
	...
	if not breakthrough_outcomes.is_empty():
		notes.append("突破链记录了关键选择、关键物消耗、结果与失败原因标签，并写入轮回账本占位。")
	return notes
