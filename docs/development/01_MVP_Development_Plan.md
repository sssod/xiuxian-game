...
| Runtime architecture | In progress | 75% | Runtime skeletons for room, world time, command queue, character state, settlement, result package, replay, data, save, logging, and UI adapter added; save/load recovery runner, save-slot diagnostics, first-entry character initialization, command validation, fallback, and hourly cultivation settlement added | Add scenario-focused settlement APIs as P3/P4 systems arrive | None |
...
| Character/realm/cultivation | In progress | 35% | Config-backed true-spirit/current-life initialization added with identity, attributes, aptitude, derived bars, lifespan, initial cultivation state, replay event, save/load preservation, debug UI summary, initial command profiles, active-breathing hourly CP settlement, Qi Refining segment progress, minor-stage advancement, and low-value fallback meditation | Add seclusion, consolidation, managed action, method study, recovery semantics, and resource input bindings as inventory/resource state arrives | Final balance values still need later config tables |
...
| UI integration | In progress | 25% | Low-fidelity debug boot scene now exposes build/config, room/time, character summary, queue capacity, current command/fallback state, active-breathing enqueue, save-slot diagnostics, save/load results, result summary, and replay/result counts | Build cultivation and command management views as remaining P3 state APIs arrive | Needs resource input and richer command template APIs |
| Validation/replay | In progress | 45% | Empty-room smoke plus save/load/F1 repeated-tick smoke now exercise result packages, replay entries, character initialization preservation, interrupted-settlement recovery, load failure diagnostic codes, active-breathing CP gain, minor-stage advancement, and fallback after command completion | Add fixed-seed gameplay scenarios as P3/P5 systems arrive | Needs resource input and additional command semantics |
