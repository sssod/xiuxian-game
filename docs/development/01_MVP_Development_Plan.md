...
4. The first command/cultivation settlement slice implements config-backed `active_breathing`, initial Qi Refining realm segment seeds, enqueue validation, execution validation, repeated hourly `CultivationTickResult`-style output, replay entries, and low-value fallback meditation.
5. Resource input fields are present on commands; unsupported or unavailable resources are rejected, while the first config-backed active-breathing resource input now validates ownership, consumes only when the action starts, and never acts as an instant-use shortcut.
6. Minor stage advancement can occur from hourly cultivation settlement, while major breakthrough remains an explicit future command/B1 flow and is not auto-completed by reaching a CP threshold.
7. The second P3 command-template slice adds config-backed seclusion cultivation, method study, foundation consolidation, explicit managed action, and rest/recovery commands to the same queue, validation, hourly settlement, result-package, and replay path.
8. Method study writes `MethodState` mastery experience and levels without granting cultivation points; the initial MethodState is an MVP placeholder seed, not final method content or balance authority.
9. Foundation consolidation lowers meridian pressure and modestly improves foundation quality without converting residual effects into cultivation points; rest/recovery restores persistent bars and lowers pressure but remains distinct from cultivation gain.
10. Explicit managed action records a running managed-action state and can participate in F1 while active, but completion with no queued successor still drops into automatic fallback and N1.
11. The third P3 resource-input slice adds a minimal config-seeded character inventory, config-backed resource effect template, active-breathing resource binding, action-start consumption, same-stacking residual guard, compatible-hour `ActiveResourceEffect` CP release, resource-use logs, save/load preservation, and explicit consolidation clearance.
12. Execution validation is start-bound for pending commands. Already-running commands are not re-failed by their own consumed resource input or residual effect on later hourly ticks.
...
| Runtime architecture | In progress | 85% | Runtime skeletons for room, world time, command queue, character state, settlement, result package, replay, data, save, logging, and UI adapter added; save/load recovery runner, save-slot diagnostics, first-entry character initialization, command validation, fallback, hourly cultivation settlement, MethodState persistence, command-template settlement branching, resource input binding, active residual effects, and resource-use logs added | Add scenario-focused settlement APIs as P3/P4 systems arrive | None |
...
| Character/realm/cultivation | In progress | 65% | Config-backed true-spirit/current-life initialization added with identity, attributes, aptitude, derived bars, lifespan, initial cultivation state, initial MethodState, minimal inventory, replay event, save/load preservation, debug UI summary, active breathing, active-breathing resource input, seclusion cultivation, method study, consolidation, residual clearance, managed action, rest/recovery, hourly CP or mastery settlement, Qi Refining segment progress, minor-stage advancement, and low-value fallback meditation | Add recovery limits, residual decay policies, richer method/main-method bindings, and broader resource templates as inventory/resource state arrives | Final balance values still need later config tables |
...
| UI integration | In progress | 45% | Low-fidelity debug boot scene now exposes build/config, room/time, character summary, method summary, resource inventory/residual summary, queue capacity, current command/fallback/managed state, active breathing, active breathing with pill input, seclusion, method study, consolidation, residual clearance, managed action and rest enqueue controls, save-slot diagnostics, save/load results, result summary, and replay/result counts | Build cultivation and command management views as remaining P3 state APIs arrive | Needs richer command editing APIs and non-debug resource selection UI |
| Validation/replay | In progress | 70% | Empty-room smoke plus save/load/F1 repeated-tick smoke now exercise result packages, replay entries, character initialization preservation, interrupted-settlement recovery, load failure diagnostic codes, active-breathing CP gain, minor-stage advancement, command-template settlement, resource-input ownership rejection, action-start consumption, active residual persistence through save/load, same-group residual stacking rejection, compatible-hour resource CP release, consolidation residual clearance without CP gain, MethodState mastery gain, consolidation pressure/foundation changes, managed-action state, rest recovery, and fallback after command completion | Add fixed-seed gameplay scenarios as P3/P5 systems arrive | Needs broader resource templates, residual decay policies, and recovery-limit semantics |
...
| 2026-05-12 | Advanced P3 with minimal config-backed cultivation resource input, active residual effects, explicit residual clearance, debug UI resource controls, and smoke coverage. |
