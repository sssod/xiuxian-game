...
- Special rule for calibrating `docs/inbox/xiuxian_design_reorganized_md`: this rule overrides the general source-handling bullets above only for the final written form of documents in this directory. During calibration, canonical docs, other inbox files, Figma, and external material may be consulted according to the normal source-handling rules. The calibrated documents in `docs/inbox/xiuxian_design_reorganized_md` must be self-contained as a directory package after editing: documents inside this directory package may reference each other, but they must not reference or depend on content from any document outside this directory. Any imported decision or detail from outside this directory must be restated in a document within this directory package, and the package must not leave external source lists, citations, or unresolved dependencies that require material outside this directory to interpret.
